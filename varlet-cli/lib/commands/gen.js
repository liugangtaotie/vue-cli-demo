...
function removeFiles(dest) {
    ...
    files.forEach(function (filename) { return fs_extra_1.removeSync(path_1.resolve(dest, filename)); });
}
function syncVersion(name) {
    var file = path_1.resolve(constant_1.CWD, name, 'package.json');
    var pkg = JSON.parse(fs_extra_1.readFileSync(file, 'utf-8'));
    var cliPkg = JSON.parse(fs_extra_1.readFileSync(constant_1.CLI_PACKAGE_JSON, 'utf-8'));
    pkg.devDependencies['@varlet/cli'] = "^" + cliPkg.version;
    pkg.files = ['es', 'umd', 'highlight', 'types', 'README.md'];
    fs_extra_1.writeFileSync(file, JSON.stringify(pkg, null, 2));
}
function gen(name) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    dest = path_1.resolve(constant_1.CWD, name);
                ...
                case 1:
                    ...
                    generator = path_1.resolve(constant_1.GENERATORS_DIR, choice);
                    return [4 /*yield*/, fs_extra_1.copy(generator, dest)];
                case 2:
                    _a.sent();
                    removeFiles(dest);
                    syncVersion(name);
