...
function compileTags(table, tags, componentName) {
    tags[lodash_1.get(varletConfig, 'namespace') + "-" + componentName] = {
        attributes: table.attributesTable.map(function (row) { return exports.replaceDot(row[0]); }),
    };
}
...
function compileAttributes(table, attributes, componentName) {
    table.attributesTable.forEach(function (row) {
        var attrNamespace = lodash_1.get(varletConfig, 'namespace') + "-" + componentName + "/" + exports.replaceDot(row[0]);
        attributes[attrNamespace] = {
            type: exports.replaceUnderline(row[2]),
            description: row[1] + " \u9ED8\u8BA4\u503C\uFF1A" + exports.replaceDot(row[3]),
        };
    });
}
...
function compileWebTypes(table, webTypes, componentName) {
    ...
    var attributes = attributesTable.map(function (row) { return ({
        name: exports.replaceDot(row[0]),
        description: row[1],
        default: exports.replaceDot(row[3]),
        value: {
            type: exports.replaceUnderline(row[2]),
            kind: 'expression',
        },
    ...
    webTypes.contributions.html.tags.push({
        name: lodash_1.get(varletConfig, 'namespace') + "-" + componentName,
...
function compileMD(path, tags, attributes, webTypes) {
    ...
    var md = fs_extra_1.readFileSync(path, 'utf-8');
...
function compileDir(path, tags, attributes, webTypes) {
    var dir = fs_extra_1.readdirSync(path);
    dir.forEach(function (filename) {
        var filePath = path_1.resolve(path, filename);
        fsUtils_1.isDir(filePath) && compileDir(filePath, tags, attributes, webTypes);
        fsUtils_1.isMD(filePath) && compileMD(filePath, tags, attributes, webTypes);
    });
}
...
function compileTemplateHighlight() {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, fs_extra_1.ensureDir(constant_1.HL_DIR)];
                case 1:
                    _a.sent();
                    tags = {};
                    attributes = {};
                    webTypes = {
                        ...
                        framework: 'vue',
                        version: '1.0.0',
                        name: lodash_1.get(varletConfig, 'title'),
                        contributions: {
                            html: {
                                tags: [],
                    ...
                    return [4 /*yield*/, Promise.all([
                            fs_extra_1.writeFile(constant_1.HL_WEB_TYPES_JSON, JSON.stringify(webTypes, null, 2)),
                            fs_extra_1.writeFile(constant_1.HL_TAGS_JSON, JSON.stringify(tags, null, 2)),
                            fs_extra_1.writeFile(constant_1.HL_ATTRIBUTES_JSON, JSON.stringify(attributes, null, 2)),
                        ])];
                case 2:
                    _a.sent();
                    return [2 /*return*/];
